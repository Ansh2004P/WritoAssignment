...
const DynamicTable = () => {
  ...
  const deleteRow = (index) => {
    ...
    setOriginalRowsData((prevRowsData) => prevRowsData.filter((_, i) => i !== index)); // Update original rows data
  };
  ...
  const addColumn = () => {
    ...
    setOriginalRowsData((prevRowsData) => prevRowsData.map((row) => [...row, []])); // Update original rows data
                  ...
                  {row.map((data, index2) => (
                    ...
                    >
                      <input
                        type="text"
                        value={data.join(",")}
                        onChange={(e) => handleChange(e.target.value, i, index2)}
